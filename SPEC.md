...
├── bundle-reach-binaries.js       ← split slot cache into one per-outcome binary per locked entry (data/reach/, ~3MB gz total, ~80KB median per outcome)
...
│   └── reach/                     ← per-outcome reachability binaries (gzipped, one per locked entry)
...
`/explore`, the reach precompute) shares these exact predicates,
...
- `physical_rate` is auto-moved to `flavor` post-module via `attachModuleReducer` (`nodeIds \ writes`); `knowledge_rate` and `failure_mode` stay in `sel` (in `ROLLOUT_WRITES`) because outcome `reachable` clauses and `primaryDimension` lookups are sel-only by contract. `rollout_set` serves as the single re-ask guard for all three nodes via `hideWhen`. Narrative / flavor blocks read `physical_rate` through `narrEff` (fused view).
...
- **Outcome-relevant dims stay in `writes`; pure-narrative dims auto-move to flavor.** `knowledge_rate` is the `primaryDimension` for several outcome variants (the-gilded-singularity / the-new-hierarchy / the-flourishing on the asi path; the-plateau / the-automation on plateau/agi); `failure_mode` is read by sel-only `reachable` clauses on the main path. Both must persist in `sel` post-exit, so they're in `ROLLOUT_WRITES`. `physical_rate` has no sel-only consumers — only narrative / flavor blocks read it via `narrEff` — so it's the single auto-moved dim (`nodeIds \ writes = { physical_rate }`). `rollout_set` is the post-answer hide gate via `hideWhen` on each of the three nodes.
