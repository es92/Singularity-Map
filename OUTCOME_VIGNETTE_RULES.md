# Outcome Vignette Writing Rules

Working notes for rewriting `data/outcomes2.json` outcome/world vignettes.

## Scope

- These rules apply to outcome vignette text: the `flavors` copy in `data/outcomes.json` and overlay rewrites in `data/outcomes2.json`.
- Do not use these rules to change outcome titles, subtitles, summaries, reachability, routing, or personal/professional vignettes unless we explicitly decide to broaden the scope.
- `outcomes2` should remain a sparse overlay: only include revised vignette fields that differ from the original.

## Core Principles

- State the selected condition in retrospective narrative language without adding new facts.
- Prefer plain past-tense observation over causal micro-stories or analysis voice.
- Avoid inventing actors, intentions, sequences, institutions, mechanisms, or social arrangements that were not selected by the path.
...
- Hidden observers or followers: "others watched," "latecomers followed," unless directly selected.
- Unchosen social structures: "within each fragment," "each civilization defined fairness," "communities solved their own problems," unless directly supported.
...
1. Is every concrete claim traceable to a selected answer or the outcome itself?
2. Would this still read correctly across multiple paths that share this value?
