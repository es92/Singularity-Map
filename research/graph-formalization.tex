...
\texttt{collapseToFlavor.when} rule: every dimension $d$ referenced
by $\varphi$ must exist ($d \in \Dim$), and every value referenced
must be a valid edge ($v \in \Edg{d}$). The same holds for
\texttt{collapseToFlavor.set} / \texttt{setFlavor} write maps,
...
\texttt{collapseToFlavor.set} / \texttt{setFlavor}) or via a module's
...
10 edges, with an upstream edge whose \texttt{collapseToFlavor.set}
...
\texttt{collapseToFlavor.when} conditions preserved. Its 1204 baseline states compress to 20 irrelevance
