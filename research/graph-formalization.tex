...
\newcommand{\cleannf}{\mathsf{C}_{\neg f}} % clean without autoForce
...
\[
  ...
  \begin{cases}
    \ell_d(\sel^{(i)})
      & \text{if } d \text{ is locked in } \sel^{(i)}
        \text{ and autoForce is active},\\
    \bot
      & \text{if } \sel^{(i)}(d) \neq \bot \text{ and }
        \Dis_{d,\sel^{(i)}(d)}(\sel^{(i)}) = 1,\\
    \bot
      & \text{if } d \text{ was auto-forced and is no longer visible
              and hidden},\\
...
We write $\cleannf$ for the variant with autoForce disabled (no locking
step), which only removes disabled choices.
\end{definition}
...
\Procedure{BaselineDFS}{$\sel$}
  ...
  \ForAll{$e \in \Edg{d}$ with $\Dis_{d,e}(\sel)=0$}
    \State $\sel' \gets \cleannf(\sel[d \mapsto e])$
...
At runtime, given the user's current explicit choice stack and a candidate
move $(d,e)$:

\begin{enumerate}
  \item Reconstruct a \emph{raw} state $\sel_{\mathrm{raw}}$ from
        explicit stack entries only (excluding auto-forced values).
  \item Apply $\cleannf(\sel_{\mathrm{raw}}[d \mapsto e])$ to get the
        hypothetical next state $\sel'$.
  \item Compute $\ik(\sel')$ and check membership in the preloaded
        $\mathsf{ReachSet}_k$.
\end{enumerate}

The use of $\cleannf$ (no auto-force) on both the server DFS and the
client lookup ensures key agreement.
...
\texttt{deriveWhen} rule: every dimension $d$ referenced by $\varphi$
must exist ($d \in \Dim$), and every value referenced must be a valid
edge ($v \in \Edg{d}$). The same holds for derivation outputs
(\texttt{value}, \texttt{valueMap}, \texttt{fromState}) and outcome
template conditions.
\end{definition}
...
\begin{definition}[Click-Erased Edge]\label{def:clickerased}
Edge $e$ of dimension $d$ is \textbf{click-erased} at state $\sel$ if
pushing $(d,e)$ onto the stack and applying $\clean$ (with autoForce)
results in a state where $d$ is no longer set:
\[
  \clean(\sel[d \mapsto e])(d) = \bot.
\]
This means the user's click is immediately undone by the auto-force /
clean logic, which is a UX anomaly.
\end{definition}
...
\Procedure{ClassDFS}{$\sel$}
  ...
  \ForAll{$e \in \Edg{d}$ with $\Dis_{d,e}(\sel)=0$}
    \If{$[e]_d \notin \mathit{seen}$}
      ...
      \State $\sel' \gets \cleannf(\sel[d \mapsto \Rep_d([e]_d)])$
...
space. In the Singularity Map, this reduces ${\sim}285\text{k}$ visited
states to ${\sim}239\text{k}$ distinct irrelevance keys.
\end{remark}
...
\Procedure{SuperDFS}{$\sel, \Sigma$}
  ...
  \ForAll{$d \in \Sigma$}
    \If{$\mathrm{NeedsCollapse}(d, \sel, d^*)$}
      ...
      \ForAll{$r \in \Rep_d$} \Comment{branch on class reps of $d$}
        \State $\sel' \gets \cleannf(\sel[d \mapsto r])$
  ...
  \If{only one class is enabled}
    \State $\mathit{child} \gets \Call{SuperDFS}{\cleannf(\sel[d^* \mapsto e]),\;\Sigma}$
  \ElsIf{$d^*$ is superimposable at $\sel$}
    \State $\mathit{child} \gets \Call{SuperDFS}{\cleannf(\sel[d^* \mapsto \Rep_{d^*}(C_1)]),\;
           \Sigma \cup \{d^*\}}$
  \Else
    ...
    \ForAll{class representatives $r$ of enabled edges of $d^*$}
      \State $\mathit{child} \gets \mathit{child} \mathbin{|}
             \Call{SuperDFS}{\cleannf(\sel[d^* \mapsto r]),\;\Sigma}$
    \EndFor
  \EndIf
...
terminates the loop. Each change either deletes a value or forces a locked
value; since forcing can trigger at most one deletion cascade per
dimension, and there are finitely many dimensions, the iteration
converges.
\end{proof}
...
\[
  \bigvee_{i=1}^{m}
  \Reach\bigl(\cleannf(\sel[d \mapsto \Rep_d(C_i)]),\, k\bigr)
  \;=\;
  \Reach\bigl(\cleannf(\sel[d \mapsto \Rep_d(C_1)]),\, k\bigr).
\]
...
\begin{lemma}[AutoForce Agreement]\label{lem:autoforce}
Let $\sel_{\text{full}} = \clean(\sel_0[d_1\mapsto e_1]\cdots[d_t\mapsto e_t])$
be the state produced by the browser's $\mathsf{push}$ (with autoForce),
and let $\sel_{\text{raw}}$ be the state reconstructed from only the
explicit entries $(d_1,e_1),\dots,(d_t,e_t)$ with $\cleannf$ applied.
Then:
\[
  \ik(\sel_{\text{raw}}) = \ik(\sel_{\text{DFS}})
\]
where $\sel_{\text{DFS}}$ is the corresponding state visited by the
server-side DFS (which also uses $\cleannf$). The browser's
$\mathsf{\_getRawSel}$ strips auto-forced dimensions, ensuring the
irrelevance key matches the DFS's key space.
\end{lemma}
...
\begin{remark}[Derivation-Induced Path Crossovers]\label{rem:crossover}
A \texttt{deriveWhen} rule on dimension $d_1$ that references dimension
$d_2$ (where $d_2$ appears later in the ordering and is conditionally
activated by $d_1$) can create \textbf{path crossovers}: a choice of
$d_2$ changes the resolved value of $d_1$, which in turn changes the
visibility of downstream dimensions, effectively routing the state from
one ``branch'' of the graph into another.

For example, if $d_1 = \mathsf{capability}$ has edge values
$\{\mathsf{singularity}, \mathsf{stalls}\}$ and
\texttt{deriveWhen} rule ``if $d_2 = \mathsf{stall\_recovery}$ is
$\mathsf{mild}$, then $d_1$ resolves to $\mathsf{singularity}$'', then
the user can select $d_1 = \mathsf{stalls}$ but later selecting
$d_2 = \mathsf{mild}$ causes $d_1$ to resolve to $\mathsf{singularity}$,
activating entirely different downstream dimensions.

This crossover is correctly handled by the class computation (R1), which
gives $d_2$ distinct classes for values that produce different derivation
outcomes, and by the fingerprint check (R3), which prevents superposition
when different class representatives produce different visible-node sets.
The terminal memoization bug (Remark~\ref{rem:termmemo}) was the only
mechanism by which crossover effects leaked incorrectly, and is resolved
by the corrected algorithm ordering.
\end{remark}
...
For safe push dimensions, $\cleannf(\sel[d\mapsto e]) = \sel[d\mapsto e]$
...
\begin{enumerate}
  ...
  \item \textbf{Comparison.} Baseline states are aggregated by
    $\ik$---all baseline masks sharing the same irrelevance key are
    OR-combined. Then:
    \begin{itemize}
      \item \textbf{No false positives:} every bit in
        $\mathsf{ReachMap}(\ik)$ must appear in the baseline aggregate
        at $\ik$.
      \item \textbf{No false negatives:} every bit in the baseline
        aggregate must appear in $\mathsf{ReachMap}(\ik)$ (with a
        documented exception for \texttt{noClassMergeDims} that are
        unset in the selector).
    \end{itemize}
\end{enumerate}
...
\texttt{irrelevance}    & 3  & 19     & 4  & R2: irrelevance wildcarding \\
\texttt{superposition}  & 4  & 36     & 5  & R3: superposition \\
\texttt{variants}       & 3  & 22     & 4  & R4: variant expansion \\
\texttt{kitchen-sink}   & 7  & 119    & 7  & Combined features \\
\texttt{derive-crossover} & 5  & 11   & 5  & \texttt{deriveWhen} crossover \\
\texttt{production-subgraph} & 16 & 6700 & 12
  & Real graph (capability $\to$ open\_source) \\
\end{tabular}
\end{center}

The \texttt{derive-crossover} graph is the minimal reproducer for the
terminal memoization bug (Remark~\ref{rem:termmemo}): 5 dimensions, 10
edges, with a \texttt{deriveWhen} rule that creates a path crossover.
Before the fix (moving the terminal check before memoization), it
produced both false positives and false negatives. After the fix, all
8 graphs pass.
...
\texttt{disabledWhen}, \texttt{requires}, and \texttt{deriveWhen}
conditions preserved. Its 6700 baseline states compress to 12 irrelevance
...
(${\sim}200\text{k}+$ states) within reasonable time, so the full graph
is validated only by the \textsf{walk} procedure's internal invariant
checks, not by baseline comparison.
...
\ref{lem:autoforce}
  & Lemma
  & Client raw-selection reconstruction matches DFS keys. \\
\ref{lem:safepush}
  & Lemma
...
\ref{def:clickerased}
  & Defn
  & Click-erased: autoForce undoes the user's choice. \\
\ref{def:stackint}
  & Defn
