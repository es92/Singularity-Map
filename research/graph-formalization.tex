...
\Procedure{SuperDFS}{$\sel, \Sigma$}
  ...
  \ForAll{$d \in \Sigma$}
    ...
    \EndIf
  \EndFor
  \State Deduplicate enabled edges of $d^*$ to one per class
  \If{only one class is enabled}
    \State \Return $\Call{SuperDFS}{\cleannf(\sel[d^* \mapsto e]),\;\Sigma}$
  \ElsIf{$d^*$ is superimposable at $\sel$}
    \State \Return $\Call{SuperDFS}{\cleannf(\sel[d^* \mapsto \Rep_{d^*}(C_1)]),\;
           \Sigma \cup \{d^*\}}$
  \Else
    \State $\mathit{mask} \gets 0$
...
\noindent
This decomposes into the four reduction theorems (R1--R4) and six
supporting lemmas listed below. The validation invariants define what
``same results'' means concretely.
